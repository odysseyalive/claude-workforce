# Claude Workforce: The Full Theory

*Everything this system believes, why it believes it, and how each piece was built. This is the long version. The [README](README.md) is the short one.*
...
What separation doesn't catch is a single agent spinning its wheels: reading the same file a third time, re-running the command that already failed, thrashing in a way that piles up history without moving the work. Nothing that ships stops that while it happens. `wf-loop-guard` was built to watch for it, and it was removed in the simplification release without ever having been switched on, because a detector nobody wires catches nothing. The nearest thing that ships is `wf-turn-ledger`, which prints how many reads a turn made and how many were distinct, so one file read three times shows as one source and not three. A stuck agent reviewing its own stuck context is the worst-placed judge there is; the useful second look comes from a different reader, which is why the audit's panels are built to disagree.
