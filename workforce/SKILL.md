...
3b. **UNAVAILABLE IS NOT FAIL, and it is not PASS.** The spawn did not happen — the host suppresses subagent spawning by ambient instruction, measured by attempting one throwaway spawn and NEVER by reading a config key or flag name (references/staging.md § UNAVAILABLE). Register DEGRADED: mark every handbook `Release Record: not probed (spawning unavailable)`, and name the remedy — the blocking instruction's own condition is *unless the user asks*, so an explicit request in the project's `CLAUDE.md` satisfies it. **IF any report, chart, or `EMP` file shows an unprobed handbook as released, or leaves the field blank where a verdict belongs → STOP and correct it.** A blank reads as a pass.
