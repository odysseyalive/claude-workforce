...
<!-- Enforcement (maintainer-facing; bin/ does not ship — on a host this is `/workforce verify`): 11 assertion(s) in bin/check name this file; 30 normative claims total. 8 generic assertions guard it too. Coverage is a floor, not a certificate. -->
...
| 12 | `model:` and `effort:` are per-agent frontmatter; `model:` defaults to `inherit`; resolution order is `CLAUDE_CODE_SUBAGENT_MODEL` → per-invocation → frontmatter → session model | Every employee is model-pinned, which is why `/org` never needs a lane check or a model-switch prompt | **canaried by self-report on 2.1.245 (2026-08-27), best-effort** — `measurements/2026-08-27-applied-model.md`. Two forced spawns (haiku-pinned, opus-pinned) each self-reported the *forced* model exactly, so the frontmatter path applies. This is a SELF-REPORT measurement, softer than fact 4b's tool-grant observation — a model introspecting its own ID — so the canary that re-checks it (`staging.md` § Phase D) is ADVISORY and may not block. **Negative finding, recorded so it is not re-hunted:** there is NO model-identifying env channel a subagent can read — `CLAUDE_CODE_SUBAGENT_MODEL`, `CLAUDE_MODEL`, and `ANTHROPIC_MODEL` were all empty even in the overridden spawn; self-report is the only channel. The `effort:` half is fact 12b and remains unverified |
...
| 12c | **`claude-fable-5-1` accepts all five effort rungs — `low` / `medium` / `high` / `xhigh` / `max`. The code lane's pick has the identical ladder; `claude-opus-5-5` accepts all five effort rungs — `low` / `medium` / `high` / `xhigh` / `max`. Its DEFAULT rung is `medium`, not `high` — the one model in this pool whose unset-effort behavior differs from every other, and a change from the `claude-opus-5` this lane ran until 2026-09-22, whose default was `high`.** Unlike `claude-opus-4-6` (no `xhigh`) and `claude-haiku-4-5` (rejects `effort` entirely), both of those models span the full ladder, exactly as `claude-fable-5` did | The `creative-visual` lane and the session advisor pin `claude-fable-5-1`, and the `code` lane pins `claude-opus-5-5` (`org-config.template.md` § Model statics, rows 1 and 4, since 2026-09-22), so Step 0.4b's `creative-visual` effort object and Step 0.4c's `code` object are rendered from these rung sets — identical sets, which is why neither of the code lane's two repins changed an offer: creative-visual recommended `medium`, offering the four nearest it (`xhigh`/`high`/`medium`/`low`, dropping the far `max` end per audit-setup.md § Step 0.4b); code recommended `high`, offering `max`/`xhigh`/`high`/`medium`. **The differing default changes what the ladder does not.** The rungs offered are the same four, but a reader who calls code's `high` "the API default" is now wrong: on this pick `high` is one rung ABOVE the default, so `org-config.template.md` § Effort statics states it as a deliberate raise. Without an established rung set the effort budget cannot render either object — this was blocker 3b of the creative-lane split | **DOCUMENTED, not measured** — read from the `claude-api` skill's model reference (the Thinking & Effort per-model table, and the Fable 5.1 section: *"Control depth with `output_config.effort` (supports `low` through `xhigh` and `max`)"*), cached 2026-06-24, which sources the Anthropic model reference; re-read 2026-09-02 when the pick moved from `claude-fable-5` to `claude-fable-5-1`, which that table lists with the identical ladder, and again 2026-09-05 when the code lane moved to `claude-opus-5`, which that same table lists at `low`-`max`, all five. **Re-read 2026-09-22** when the code lane moved to `claude-opus-5-5`: that table's `Claude Opus 5.5` row reads "`low`/`medium`/`high`/`xhigh`/`max` — **default `medium`** (not `high`); per-message effort (beta) supported", which is where the differing default comes from — it is the table's own words, not an inference from the price change. The same read gives the rest of the row this pool depends on: $4/$20 per MTok, 1M context, 128K max output. **No behavioral canary was run on this host:** effort acceptance is not observable from a spawned agent's self-report the way a tool grant is (fact 4b), so per fact 12b's own rule and Operating Principle 13 this stays DOCUMENTED and **may not become a blocking check** until a canary confirms it. Added 2026-08-23 for the creative-lane split (blocker 3b); re-pointed 2026-09-02, and again 2026-09-22 |
| 12d | **`claude-opus-4-8` accepts all five effort rungs — `low` / `medium` / `high` / `xhigh` / `max`.** It spans the full ladder, like `claude-opus-5-5` and `claude-fable-5-1` above and unlike `claude-opus-4-6` (no `xhigh`) and `claude-haiku-4-5` (rejects `effort` entirely). **It also keeps the `high` effort default, which the code lane's `claude-opus-5-5` does not** (fact 12c) — so since 2026-09-22 the two analytical rows and the code row no longer share one unset-effort behavior. Its thinking surface differs from `claude-opus-5-5` in a way effort does not: `{type: "adaptive"}` is the only on-mode and omitting `thinking` runs *without* it, where Opus 5.5 thinks by default | The `analytical` lane pins `claude-opus-4-8` at BOTH tiers — Lead at `high`, IC at `medium` (`org-config.template.md` § Model statics row 2 — row 3 until the 2026-09-22 reorder, § Recommended effort per lane, and the Tier/Model/Effort table). Analytical is the default lane, so an employee with no lane override resolves here: this is the rung set most employees in a generated org are budgeted against, and it was the one lane fact 12c did not cover. Without it the effort budget renders the analytical objects against no established ladder — the same blocker 3b shape the creative-lane split hit, one lane over | **DOCUMENTED, not measured** — read from the `claude-api` skill's model reference (the Thinking & Effort per-model table, whose `Opus 4.8 / 4.7` row lists `low`/`medium`/`high`/`xhigh`/`max` and states `budget_tokens` is removed, 400), cached 2026-06-24, which sources the Anthropic model reference. Read 2026-09-13. **No behavioral canary was run on this host:** effort acceptance is not observable from a spawned agent's self-report the way a tool grant is (`platform.md` fact 4b, § Fact 4b), so per fact 12b's own rule this stays DOCUMENTED and **may not become a blocking check** until a canary confirms it. Added 2026-09-13 — the analytical lane had been pinned to this model since 2026-09-09 with no rung set on record |
