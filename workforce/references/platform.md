...
| 12c | **`claude-fable-5-1` accepts all five effort rungs — `low` / `medium` / `high` / `xhigh` / `max`. The code lane's pick has the identical ladder; `claude-opus-5` accepts all five effort rungs — `low` / `medium` / `high` / `xhigh` / `max`.** Unlike `claude-opus-4-6` (no `xhigh`) and `claude-haiku-4-5` (rejects `effort` entirely), both of those models span the full ladder, exactly as `claude-fable-5` did | The `creative-visual` lane and the session advisor pin `claude-fable-5-1`, and the `code` lane pins `claude-opus-5` (`org-config.template.md` § Model statics, rows 1 and 2, since 2026-09-05), so Step 0.4b's `creative-visual` effort object and Step 0.4c's `code` object are rendered from these rung sets — identical sets, which is why the code lane's repin changed no offer: creative-visual recommended `medium`, offering the four nearest it (`xhigh`/`high`/`medium`/`low`, dropping the far `max` end per audit-setup.md § Step 0.4b); code recommended `high`, offering `max`/`xhigh`/`high`/`medium`. Without an established rung set the effort budget cannot render either object — this was blocker 3b of the creative-lane split | **DOCUMENTED, not measured** — read from the `claude-api` skill's model reference (the Thinking & Effort per-model table, and the Fable 5.1 section: *"Control depth with `output_config.effort` (supports `low` through `xhigh` and `max`)"*), cached 2026-06-24, which sources the Anthropic model reference; re-read 2026-09-02 when the pick moved from `claude-fable-5` to `claude-fable-5-1`, which that table lists with the identical ladder, and again 2026-09-05 when the code lane moved to `claude-opus-5`, which that same table lists at `low`-`max`, all five. **No behavioral canary was run on this host:** effort acceptance is not observable from a spawned agent's self-report the way a tool grant is (fact 4b), so per fact 12b's own rule and Operating Principle 13 this stays DOCUMENTED and **may not become a blocking check** until a canary confirms it. Added 2026-08-23 for the creative-lane split (blocker 3b); re-pointed 2026-09-02 |
| 12d | **`claude-opus-4-8` accepts all five effort rungs — `low` / `medium` / `high` / `xhigh` / `max`.** It spans the full ladder, like `claude-opus-5` and `claude-fable-5-1` above and unlike `claude-opus-4-6` (no `xhigh`) and `claude-haiku-4-5` (rejects `effort` entirely). Its thinking surface differs from `claude-opus-5` in a way effort does not: `{type: "adaptive"}` is the only on-mode and omitting `thinking` runs *without* it, where Opus 5 thinks by default | The `analytical` lane pins `claude-opus-4-8` at BOTH tiers — Lead at `high`, IC at `medium` (`org-config.template.md` § Model statics row 3, § Recommended effort per lane, and the Tier/Model/Effort table). Analytical is the default lane, so an employee with no lane override resolves here: this is the rung set most employees in a generated org are budgeted against, and it was the one lane fact 12c did not cover. Without it the effort budget renders the analytical objects against no established ladder — the same blocker 3b shape the creative-lane split hit, one lane over | **DOCUMENTED, not measured** — read from the `claude-api` skill's model reference (the Thinking & Effort per-model table, whose `Opus 4.8 / 4.7` row lists `low`/`medium`/`high`/`xhigh`/`max` and states `budget_tokens` is removed, 400), cached 2026-06-24, which sources the Anthropic model reference. Read 2026-09-13. **No behavioral canary was run on this host:** effort acceptance is not observable from a spawned agent's self-report the way a tool grant is (`platform.md` fact 4b, § Fact 4b), so per fact 12b's own rule this stays DOCUMENTED and **may not become a blocking check** until a canary confirms it. Added 2026-09-13 — the analytical lane had been pinned to this model since 2026-09-09 with no rung set on record |
