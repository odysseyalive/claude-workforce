...
| 12c | **`claude-fable-5` accepts all five effort rungs — `low` / `medium` / `high` / `xhigh` / `max`.** Unlike `claude-opus-4-6` (no `xhigh`) and `claude-haiku-4-5` (rejects `effort` entirely), the frontier visual model spans the full ladder | The `creative-visual` lane pins `claude-fable-5` (`org-config.template.md` § The four lanes, the out-of-pool creative-visual pick offered as this lane's leading budget option), so Step 0.4b's `creative-visual` effort object is rendered from this rung set — recommended `medium`, offering the four nearest it (`xhigh`/`high`/`medium`/`low`, dropping the far `max` end per audit-setup.md § Step 0.4b). Without an established rung set the effort budget cannot render a `creative-visual` object at all — this is blocker 3b of the creative-lane split | **DOCUMENTED, not measured** — read from the `claude-api` skill's model reference (the Thinking & Effort per-model table, and the Fable 5 section: *"Control depth with `output_config.effort` (supports `low` through `xhigh` and `max`)"*), cached 2026-06-24, which sources the Anthropic model reference. **No behavioral canary was run on this host:** effort acceptance is not observable from a spawned agent's self-report the way a tool grant is (fact 4b), so per fact 12b's own rule and Operating Principle 13 this stays DOCUMENTED and **may not become a blocking check** until a canary confirms it. Added 2026-08-23 for the creative-lane split (blocker 3b) |
