...
<!-- Enforcement (maintainer-facing; bin/ does not ship — on a host this is `/workforce verify`): 8 assertion(s) in bin/check name this file; 20 normative claims total. 8 generic assertions guard it too. Coverage is a floor, not a certificate. -->
...
| A question a human reads uses no term from the banned list — the words this distribution writes for agents stop at the user | — | **CANNOT. Detection only.** `wf-plain-guard` held this row and was removed on the user's explicit marks in the simplification release (`changes/1.31.0.md`). The rule still governs, carried by the output style and by operating-principles item 13; nothing blocks the call. |
| A budget picker offers the option set the shipped template defines, never one rebuilt from a stale project config | — | **CANNOT. Detection only.** `wf-budget-guard` held this row and went with the two budget producers it read (`changes/1.31.0.md`); tested before removal, it returned "passing through" on every call — the dormant shape this file refuses. The sole-source rule is stated at `audit-setup.md` § Step 0.4 and enforced by nothing. |
