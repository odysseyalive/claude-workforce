...
this project just closed comes back. **Row 23 landed 2026-08-26 and is the first row owed to the
companion gate** rather than to conversion, the sweep, or a guard: a real audit twice left the generated
...
**Row 23 is the companion gate's own report, and its predicate is fixed** (the shared contract
`audit-setup.md` § Step 0.3 owes and `bin/check` enforces): **after any `audit` run, the three Core
companions exist at `${CLAUDE_PROJECT_DIR}/.claude/skills/<name>/SKILL.md` for name ∈ {org,
operating-principles, personnel-ledger}, and the three evaluators — code-evaluator, text-eval,
security-evaluator — are reachable in EITHER scope, project first. Absence of any is
a FAIL, not a report** — the Core companions are materialized from shipped doctrine, and *"the
distribution ships none"* is never a valid skip for them; an evaluator reachable in no scope is a
`/workforce update` remedy, never a copy written into the project. `/org` specifically is generated by `org index`; its absence after the run
is a hard failure, not a shadowing-copy finding (`verify.md` § Install and scope). The row is
`NOT UPHELD` when a companion is absent and no rule refused it — the same uncited-refusal shape as rows
17 and 20 — so a run that materialized none cannot close clean by staying silent. Measured on a fresh
project twice: the generated `/org` was never created at `.claude/skills/org/SKILL.md`, because Step 0.3
was framed as a copy-install of a directory the distribution does not ship, and nothing counted the
companions the run owed.

**It recurred on 2026-09-11, on a fresh project on a remote host, after a two-hour audit — and the
reason it recurred is the part worth keeping.** Counting the companions was the fix that shipped, and
counting is not creating. Both mechanisms that touched the path declined the absent case on purpose:
`wf-companion` refused it to avoid giving absence "two owners that can disagree", and `wf-conform` only
tallied it afterwards. Since Step 0.3 was prose, absence had **zero** owners, and `wf-companion` printed
`ABSENT` and exited **0** for it. **`wf-companion --execute` is now the producer** — it materializes all
three Core companions from shipped source, reads each back, rejects one whose frontmatter would not
load, and exits non-zero on a Core absence in report mode too. **And the row's own scope was wrong,
which is why the recurrence was invisible**: it demanded all six project-local while Step 6-F of the
same audit retires the project's evaluator copies, so the row was RED ON EVERY HEALTHY TREE and an
absent `/org` arrived as a fourth name in a row nobody had reason to read. A detector that cries wolf
is not a weaker detector than none — it is the same as none, and it costs a release to find out.
...
Run Invariants
  ...
  INV-COMPANIONS  6 of 6 · project-local 3/3: org (wf-companion) · operating-principles · personnel-ledger · install-scope 3/3: code-evaluator · text-eval · security-evaluator · 0 absent
