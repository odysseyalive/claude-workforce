...
`audit-setup.md` § Step 0.3 owes and `bin/check` enforces): **after any `audit` run, every core companion
skill exists at `${CLAUDE_PROJECT_DIR}/.claude/skills/<name>/SKILL.md` for name ∈ {org,
operating-principles, personnel-ledger, code-evaluator, text-eval, security-evaluator}. Absence of any is
