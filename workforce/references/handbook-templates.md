...
<!-- Enforcement (maintainer-facing; bin/ does not ship — on a host this is `/workforce verify`): 43 assertion(s) in bin/check name this file; 7 normative claims total. 8 generic assertions guard it too. Coverage is a floor, not a certificate. -->
...
startup* and costs its bytes on every spawn (`procedure-for-procedures.md`). Preload only what the employee
...
<!-- direct-reports: <lead names> | max-direct-spawns: <from procedure-for-procedures.md> -->
...
<!-- When no directive binds this employee, this section reads exactly: (none bound).
     ...
     A CREATIVE-lane role binds an AUTHORED vision directive here — its owner-vision acceptance
     criteria (hire.md Step 3c), accumulated by amend.md § Promote. For such a role (none bound) is a
     gap to close by adoption, not a resting state; for analytical/code/advisor roles it stays correct. -->

## Sources
The project's authoritative reference material this work draws on. You consult it for strategy, and
**every work order you write that touches a surface names the sources the doer must consult** — a
directing node that omits them ships the thin-surface authoring this section exists to prevent
(COMPLETENESS, below).
- `<reference root or glob>` — <what it holds>
- `<grounding/evidence root>` — <the evidence claims trace to>

**Before you report anything as missing, unrecoverable, or deleted — or dispatch on an assumption about
absent context — search these, and require the same of the node you dispatch.** If it is still
unresolved, that is a QUESTION, not a finding: return `QUESTION:`/`ESCALATE:` to whoever dispatched you,
...
- COMPLETENESS is yours as the directing node — the work order is the single point of failure, and you
  ...
  - **PLAN-READINESS (Definition of Ready).** An ambiguous or tightly-coupled order is not READY to
    dispatch until an approved PLAN with acceptance criteria exists. Apply this any-of trigger BEFORE
    you build — the order needs a plan if ANY holds: (T1) it touches more than one surface or asset-set;
    (T2) it would run more than one agent against the same files, tree, or port, or span more than one
    department; (T3) it is transformation-shaped — rebuild, redesign, re-envision, overhaul — rather
    than a bounded specified edit; or (T4) you cannot state its acceptance criteria in verifiable terms
    before dispatch. If NONE hold the order is EXEMPT: a single-IC, single-surface, one-line-acceptance
    edit dispatches directly — not everything needs a plan, and a universal plan gate is its own
    bureaucracy. The acceptance criteria you write ARE the vision the work must meet and they travel
    with the task; capturing them up front is what stops the ten-reaction thrash a plan-less rebuild
    produces (and they are the seam the vision-directives bind to).
...
the state. Narrative outside those three parts is a second work order
...
| Sections | **every section the CEO template shows, in the same order, minus none** — `Role`, `Scope`, `Directives`, `Sources`, `Chain of Command`, `Verification`, `Guardrails`, `Exit criteria`, `Escalation`, `Probe`, `Reporting` |
...
fabricate, never assert an absence you have not proven, never guess. *"I concluded it is gone"* is a
`QUESTION`, not a finding. Report `UNRECOVERABLE: <what> — searched <sources>` only after every source
returned nothing.
<!-- (none) when this role produces nothing from reference material — a pure reviewer or a mechanism
     role. An absent section is silence; "(none)" is a measurement, exactly as ## Directives. -->
