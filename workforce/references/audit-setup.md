...
<!-- Enforcement (maintainer-facing; bin/ does not ship — on a host this is `/workforce verify`): 57 assertion(s) in bin/check name this file; 80 normative claims total. 8 generic assertions guard it too. Coverage is a floor, not a certificate. -->
...
**Six companions, owed in two scopes, and only the first group is this step's to write.** No question,
no checklist.
...
**The three Core companions are MATERIALIZED into the project from doctrine** — generated or written out
of a reference body, never copied from a shipped skill directory, because none of the three ships as
one. `org` is generated by `org index` (`procedures/org.md` step 2); `operating-principles` and
`personnel-ledger` are written from their reference bodies.

**The three evaluators are NOT materialized here. They ship in the user install** as complete sibling
skills beside `skills/workforce/` (`manifest.txt` § sibling; `update.md` § The five evaluators come with
it), under the user's 2026-09-10 directive that the evaluators and workforce are self-contained in the
user install and `update` is the whole propagation path. This step checks that each is REACHABLE, project
copy first, then the install; `audit` Step 6-F retires a project's own copy in the same run. An evaluator
reachable in no scope is a `/workforce update` remedy, reported by name, never written into the project.
*Corrected 2026-09-16: this step still listed the evaluators as written from `references/evaluators.md`
and said "there is no `workforce/skills/`", contradicting § `INV-COMPANIONS` below and Step 6-F since
1.52.0 (DEF-Q-18).*

**STOP — "nothing ships to copy" is NEVER a reason to skip a Core companion. Absence is not the absence
of a source — absence is the trigger to materialize it.** A literal run once reasoned *"the distribution
ships none → install none"* and skipped every companion, the mandatory `/org` among them. IF a run is
about to skip a Core companion because it found no shipped directory to copy → it has inverted the rule:
materialize it from the doctrine named above.

**`wf-companion` IS THE PRODUCER FOR AN ABSENT COMPANION. Run it; do not perform this by hand.**

```
"$WF/bin/wf-companion" --root "${CLAUDE_PROJECT_DIR:-$PWD}"            # report what is owed
"$WF/bin/wf-companion" --root "${CLAUDE_PROJECT_DIR:-$PWD}" --execute  # materialize it
```

It creates each absent Core companion from shipped source — `/org` from `templates.md` § `/org` plus
the canonical dispatch CHECKPOINT copied byte-for-byte out of `procedures/org.md`, the other two from
their reference bodies — reads each one back, checks it opens with loadable frontmatter, and **exits
non-zero when a Core companion is absent, in report mode as well as under `--execute`.** Absence is a
FAIL per `invariants.md` row 23, and the mechanism has to agree with the rule.

*Added 2026-09-11, and it is the fix for the defect this whole gate was already written against. The
gate's prose was correct and complete; the two mechanisms that touched the path both declined the
absent case — `wf-companion` said "ABSENT is not this script's job to fix. Step 0.3 materializes
companions" and returned 0, and `wf-conform` only COUNTED. Step 0.3 is prose, so absence had ZERO
owners rather than the two the refusal was avoiding. **MEASURED on a remote host: a two-hour `audit`
on a fresh project finished with no `.claude/skills/org/SKILL.md` and nothing in the run said so.*

**`/org` is GENERATED, not a companion-copy.** Do not treat it as a copy-install that "ships none."
`wf-companion` materializes it here, and `org index` — which runs UNCONDITIONALLY at audit Step 6, in
every scope — refreshes its CHECKPOINT region at
`${CLAUDE_PROJECT_DIR}/.claude/skills/org/SKILL.md` (`references/scopes.md` § The `/org` receptionist is
project-local). **Both read the same span through the same function, so the created copy and the
refreshed copy cannot drift.** This gate never reports `/org` absent-and-skipped: it either
materializes it or fails.
...
before the sweep and printed at close. **The two groups are owed in two different SCOPES and the line
says which** — the three Core companions exist at
`${CLAUDE_PROJECT_DIR}/.claude/skills/<name>/SKILL.md` (`org`, materialized by `wf-companion` and
refreshed by `org index`; `operating-principles`; `personnel-ledger`), and the three evaluators are
**reachable in either scope, project first**, because they ship in the user install and Step 6-F
retires the project's copies in this same run. *Corrected 2026-09-11: this demanded all six
project-local, so a correctly-audited project failed the row naming three evaluators the same audit had
just removed — and a row red on every healthy tree is a row nobody reads, which is how an absent `/org`
arrived as a fourth name nobody looked at.* An
...
```
INV-COMPANIONS  6 of 6 · project-local 3/3: org (wf-companion) · operating-principles · personnel-ledger
                · install-scope 3/3: code-evaluator · text-eval · security-evaluator · 0 absent
```
...
|---|---|---|
| **ABSENT** | no directory | **materialize from doctrine, via the producer** — `wf-companion --execute` creates all three Core companions (`org`, `operating-principles`, `personnel-ledger`) from shipped source and reads each back; an evaluator unreachable in EVERY scope is a `/workforce update` remedy, not an audit one. Never "install the shipped version": nothing ships to copy |
