...
## `workforce-version:` on the header

**The chart says which release built this org, in its first three lines.**

Every handbook already carries `workforce-version:` in its ORG-RECORD block — the release that
authored *that employee*. The chart did not, and the chart is what a run opens first. So the one
question `audit` must answer before it decides anything — *is this org older than the release I am
running?* — could only be answered by opening every handbook in `.claude/agents/` and taking the
minimum. On an org of nine that is nine reads to learn one number, which is why no procedure did it
and why `audit` Step 5d's heal-an-existing-org path had to key off a per-handbook stamp instead of an
org-wide one.

**It is written by `org index`, from `references/version.md`, and never by hand.** The value is the
running release — the copy of the skill that generated the chart — not the release the org was first
...
|---|---|---|
| equal | the org was last indexed by this release | ordinary re-audit |
| older | the org predates this release | run the heal passes for everything shipped in between |
| newer | the skill was downgraded under a newer org | STOP and report both numbers — never silently re-index an org built by a release this copy does not understand |
| absent | the chart predates this field | treat as older, and say so rather than assuming equal |

**Absent is not equal.** A chart written before this field existed carries no stamp, and reading that
as "current" is the failure this field exists to prevent — it is the same shape as a blank release
record reading as a pass (§ Off-the-Street Release Gate rule 3b).
...
<!-- Generated: YYYY-MM-DD | harness: claude-code <version> | workforce-version: <WORKFORCE-VERSION> -->
