...
|---|---|---|---|
| `skill-builder` | ORCHESTRATOR | removal test: minus its agents nothing remains | foreign-owned (owns 31) |
| `focus`         | RETAIN r7    | generated by skill-builder; succession not declared | foreign-owned |
| `agenda`        | SPLIT        | workflow plus a tracked dataset | — |
...
- **A later reader cannot ask why.** "Why was `/focus` left alone?" has no answer on disk, so the next
