...
| `MIGRATE` | a Core skill older than its region: shipped items still sit inside the user's span, or `personnel-ledger` has no region. **Name the remedy: `/workforce audit`**, whose `wf-companion --execute` migrates it (`audit.md` § Step 4, its paragraph on a Core skill older than its region). Not a failure |
