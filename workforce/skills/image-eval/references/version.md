...
# image-eval reference version

This file is the **drift anchor** for the image catalog, mirroring the code, text and security
catalogs. An audit reads the integer below (workforce's *shipped* image-reference version) and
compares it to the version recorded in a project's `image-eval` skill. Shipped > recorded → the
